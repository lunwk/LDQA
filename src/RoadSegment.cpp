...
namespace LD {
    ...
    RoadSegment::RoadSegment(string _xmlFile) : Solver(_xmlFile), m_DBScan(_xmlFile), m_cropSizeH(1025), m_cropSizeW(2049),
        ...
        ParseXML();
        if (m_debug) {
            cout << "Entered RoadSegment::RoadSegment()" << endl;
            std::cout << "m_dataRoot      " << m_dataRoot << std::endl;
            std::cout << "m_dataFile      " << m_dataFile << std::endl;
            std::cout << "m_RoadSegRoot       " << m_segRoot << std::endl;
            std::cout << "m_overlayedRoot " << m_overlayedRoot << std::endl;
            std::cout << "m_saveVizImg    " << m_saveVizImg << std::endl;
            std::cout << "m_vizImgPrefix  " << m_vizImgPrefix << std::endl;
            std::cout << "m_segImgPrefix  " << m_segImgPrefix << std::endl;
        }
...
/*        cv::Mat ROI(_inputImg, cv::Rect(0, m_originalH/4, m_originalW,  m_originalH*3/4));
        cv::Mat croppedImage;
        ROI.copyTo(croppedImage);
        Preprocess(croppedImage);*/
